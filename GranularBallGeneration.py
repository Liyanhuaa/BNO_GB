...
class GB:
    def __init__(self, data, label):
        self.data = data
        self.center = self.data[:,2:].mean(0)
    ...
    def get_radius(self):
        return max(((self.data[:,2:] - self.center) ** 2).sum(axis=1) ** 0.5)
...
def division(args,gb_list, gb_list_not):
    gb_list_new = []
    for gb in gb_list:
        if len(gb) > args.len_to_split_ball:
...
def spilt_ball(data):
    ...
    for i in range(len(data)):
        if dis1 < (sum((data[:,2:][i] - center) ** 2)):
            dis1 = sum((data[:,2:][i] - center) ** 2)
            dis1i = i
    for i in range(len(data)):
        if dis2 < (sum((data[:,2:][i] - data[:,2:][dis1i]) ** 2)):
            dis2 = sum((data[:,2:][i] - data[:,2:][dis1i]) ** 2)
            dis2i = i
    for j in range(0, len(data)):
        if (sum((data[:,2:][j] - data[:,2:][dis1i]) ** 2)) < (sum((data[:,2:][j] - data[:,2:][dis2i]) ** 2)):
            ball1.extend([data[j]])
        else:
            ball2.extend([data[j]])
...
def connect_ball_overlap(gb_list, c_count):
    gb_cluster = {}
    ...
    gb_cluster_all = {}
    for i in range(0, len(gb_cluster)):
        gb = GB(gb_cluster[i].data, i)
        gb_cluster_all[i] = gb


    return gb_cluster_all
...
def GBC(args,data):
    ...
    while 1:
        ...
        gb_list_temp, gb_list_not_temp = division(args,gb_list_temp, gb_list_not_temp)
    ...
    gb_list_cluster = connect_ball_overlap(gb_list_temp, 1)
    print("gb_cluster_number",len(gb_list_cluster))
